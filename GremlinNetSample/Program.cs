...
namespace GremlinNetSample
{
    ...
    class Program
    {
        ...
        static async void SubmitGremlinRequest()
        {
            ...
            using (var gremlinClient = new GremlinClient(gremlinServer, new GraphSON2Reader(), new GraphSON2Writer(), GremlinClient.GraphSON2MimeType))
            {
                foreach (var query in gremlinQueries)
                {
                    ...
                    var response = await gremlinClient.SubmitAsync<dynamic>(query.Value);

                    // Obtain resultset.
                    var resultSet = response.AsResultSet();

                    // Obtain server response status array.
                    var statusAttributes = resultSet.StatusAttributes;


                    // Print status code
                    Console.WriteLine(String.Format("\tStatusCode:\n\t{0}", statusAttributes["x-ms-status-code"]));

                    // Print charged RU/s
                    Console.WriteLine(String.Format("\tRU-Cost:\n\t{0}", statusAttributes["x-ms-request-charge"]));
